...
function M.get_rendered_buffer(buf, i, remaining, direction, is_first)
	...
	local ambiguity_list = vim.tbl_filter(function(b)
		return utils.evaluate_buf_aware_option(config.opts.tabline.bufs.overwrites.name, b)
			== utils.evaluate_buf_aware_option(config.opts.tabline.bufs.overwrites.name, buf)
	end, bufs)

	local is_ambiguos = #ambiguity_list > 1

	local buf_text, _remaining, len =
		_buffer.render_buf(buf, is_current, is_ambiguos, i < #bufs, is_first, remaining, direction)
...
--- @param first_buf integer
--- @param index_offset integer?
--- @return string part, integer total_len, integer remaining
function M.get_rendered_side(bufs, direction, max_len, first_buf, index_offset)
	local part = ""
	local total_len = 0
	...
	for i, buf in ipairs(bufs) do
		...
		local rendered_buf, remaining, len =
			M.get_rendered_buffer(buf, i + (index_offset or 0), max_len, direction, first_buf == buf.bufnr)
...
function M.get(max_len)
	...
	local part = ""
	local total_len = 0
	local first_buf = bufs[1].bufnr

	local left_side_bufs = vim.iter(vim.list_slice(bufs, 1, current_buffer_index - 1)):rev():totable()
	local right_side_bufs = vim.list_slice(bufs, current_buffer_index + 1)

	local rendered_current_buffer, remaining, len = M.get_rendered_buffer(
		bufs[current_buffer_index],
	...
	local left_side, left_len, left_remaining_len =
		M.get_rendered_side(left_side_bufs, "right-to-left", max_left_len, first_buf)
	...
	if #right_side_bufs > 0 then
		...
		if right_remaining_len > 0 then
			left_side =
				M.get_rendered_side(left_side_bufs, "right-to-left", max_left_len + right_remaining_len, first_buf)
		end
