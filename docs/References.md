...
| section_separator | string                        | String to separate buffer and tab sections.                             | `" / "`                           |
...
| max_name_len | `integer`                          | Maximum length allowed for the buffer name (after being calculated using the overwrite function). | `max_name_len = 20`                |
...
| overwrites   | `mantel-nvim.BufContentOverwrites` | Custom content for ambiguous, named, unnamed buffers, or buffer names that overflow.              | `"[No Name]"`                      |
...
| dir_root      | `mantel-nvim.BreadcrumbPart`            | Special part to be rendered as the first part, apart from the rest. Default: `''`                                                                                                                                                | `{ text = "/", len = 1, focused = true }`                   |
